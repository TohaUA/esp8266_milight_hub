...
public:
  ...
  findAlias(MiLightRemoteType deviceType, uint16_t deviceId, uint8_t groupId);
...
protected:
  ...
  static String
  validateEnum(JsonObject obj, const __FlashStringHelper* key, const char* const validValues[], size_t numValues);

  template <typename T> void setIfPresent(JsonObject obj, const __FlashStringHelper* key, T& var) {
